...
static void usage(const char *name)
{
	...
	printf("  -I		play role of init(1) in new pid-namepsace\n");
	printf("\n");
...
int do_child(void *vargv)
{
	...
	if (child < 0) {
		perror("playing init role, but fork failed");
		return -1;
	}

	if (child) {
		/* play role of init - dead children/orphans */
		do {
			child = wait(NULL);
		} while (child >= 0);
		return 0;
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "+mguUiIphz:cntf:P:")) != EOF) {
		switch (c) {
